...
Currently, plannning under uncertainity is tested only in simulation environments using` rviz` for visualization. In fact, the simulation works for any map type that is available (_Add  a new `.yaml` file in `duckietown-world` repo if a new map type is needed_). In the future, the package will be available as a `ros` package for testing on the duckiebots in a duckietown setup.
...
- [x]  Planning under uncertainty to get a shortest collision free path under current observations.
- [x]  Planning under uncertainty to get a velocity profiler for the shortest path chosen.
- [x]  Real-time visualization of duckiebots
...
Symptom: This message appears when running the `docker` container:
...
For failure in demo execution, refer to the troubleshooting section above.
...
What would a self-driving car do if the car just in front it has malfunctioning break lights? What would it do if  a kid suddenly lets go of his mother's grasp and jumps into the road? What would it do if the sensor it uses for perception break down unexpectedly? All of these situations and more may be captured if the planning algorithms are planning for such uncertainties, which include the behavior of other agents in the environment as well as our perception of the environment itself.
...
Of course, the cost function is customizable and can be change with a few lines of code; if no cost function is sent to _velocity profiler_ , it is going to use the above mentioned cost function.
...
The `ros-uplan` package contains all the necessary nodes needed for a proper simulation and a visualization of this simulation.
...
<div figure-id="fig:rviz_planning" figure-caption="`rviz`visualization of the planning and velocity profiling algorithms">
